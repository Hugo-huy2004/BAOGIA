...
describe('requireMember', () => {
  it('rejects requests with no token', async () => {
    const { res, nextCalled } = await run(requireMember, { cookies: {}, headers: {}, query: {}, body: {} });
    expect(nextCalled).toBe(false);
    expect(res.statusCode).toBe(401);
  });

  it('accepts a valid member token via Bearer header and binds identity from it', async () => {
    ...
    const { nextCalled } = await run(requireMember, req);
    expect(nextCalled).toBe(true);
    expect(req.memberEmail).toBe('alice@example.edu'); // normalized, from token not query
  });

  it('accepts a valid member token via member_jwt cookie', async () => {
  ...
  it('rejects a forged token signed with the wrong secret', async () => {
    ...
    const { res, nextCalled } = await run(requireMember, req);
    expect(nextCalled).toBe(false);
    expect(res.statusCode).toBe(401);
  });

  it('rejects an expired member token', async () => {
  ...
  it('lets an admin act on a target email from the request', async () => {
  ...
  it('accepts an admin token with no target email (id-addressed routes)', async () => {
    const adminToken = jwt.sign({ id: '1', role: 'admin' }, JWT_SECRET);
...
describe('requireCustomer', () => {
  it('rejects requests with no token', async () => {
    const { res, nextCalled } = await run(requireCustomer, { cookies: {}, headers: {}, params: { id: 'anything' } });
    expect(nextCalled).toBe(false);
    expect(res.statusCode).toBe(401);
  });

  it('binds projectId from the token, ignoring the :id in the URL (no IDOR)', async () => {
    const token = signCustomerToken('myProjectId');
    const req = { cookies: { customer_jwt: token }, headers: {}, params: { id: 'someoneElsesProjectId' } };
    const { nextCalled } = await run(requireCustomer, req);
    expect(nextCalled).toBe(true);
    expect(req.customerRole).toBe('customer');
    expect(req.projectId).toBe('myProjectId'); // from token, NOT the attacker-supplied :id
  });

  it('lets an admin act on the project named by :id', async () => {
    const adminToken = jwt.sign({ id: '1', role: 'admin' }, JWT_SECRET);
  ...
  it('rejects a forged customer token', async () => {
    const forged = jwt.sign({ projectId: 'x', role: 'customer' }, 'wrong-secret');
    const req = { cookies: { customer_jwt: forged }, headers: {}, params: { id: 'x' } };
    const { res, nextCalled } = await run(requireCustomer, req);
    expect(nextCalled).toBe(false);
    expect(res.statusCode).toBe(401);
  });

  it('rejects a member token on customer routes (wrong role)', async () => {
...
describe('requireAdmin', () => {
  it('rejects a member token on admin routes', async () => {
    const token = signMemberToken('member@example.edu');
    const req = { cookies: { jwt: token }, headers: {} };
    const { res, nextCalled } = await run(requireAdmin, req);
    expect(nextCalled).toBe(false);
    expect(res.statusCode).toBe(403);
  });
  ...
  it('accepts a valid admin token', async () => {
    ...
    const req = { cookies: { jwt: token }, headers: {} };
    const { nextCalled } = await run(requireAdmin, req);
    expect(nextCalled).toBe(true);
  });
});
