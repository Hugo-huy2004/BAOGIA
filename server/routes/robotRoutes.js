...
router.post('/stream-token', requireAdmin, async (req, res) => {
  try {
    ...
    if (!expectedPin || masterPinHeader !== expectedPin) {
      return res.status(403).json({ success: false, message: 'Xác thực Master PIN thất bại.' });
    }
...
router.put('/config', requireAdmin, async (req, res) => {
  try {
    ...
    const expectedPin = process.env.ROBOT_MASTER_PIN || '';
    if (!expectedPin || masterPinHeader !== expectedPin) {
      return res.status(403).json({ success: false, message: 'Mã Master PIN không đúng.' });
    }
