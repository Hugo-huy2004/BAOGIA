...
function NavLink({ to, active, onClick, children }) {
  return (
    <Link
      to={to}
      onClick={onClick}
      className={`relative inline-flex h-8 items-center text-[12px] font-medium leading-none tracking-wide transition-colors duration-200 select-none ${
        active
    ...
    >
      {children}
      {active && (
        <span className="absolute -bottom-[14px] left-0 right-0 h-0.5 rounded-full bg-primary" />
      )}
    </Link>
  );
}
