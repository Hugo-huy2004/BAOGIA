    ...
    <aside className="hidden md:flex flex-col w-64 shrink-0 border-r border-slate-200 dark:border-slate-800/80 bg-white dark:bg-[#12111a] sticky top-14 h-[calc(100vh-56px)] z-20 justify-between p-6 overflow-y-auto">
      <div className="space-y-6">
        <div>
          ...
          <p className="text-[10px] text-slate-400 font-bold uppercase tracking-wider mt-1">{t("adminPanel.sidebar.dashboard")}</p>
        </div>

        <nav className="space-y-1">
          {[
            { id: "users", label: t("adminPanel.sidebar.users"), icon: "group", count: counts.users },
            { id: "bookings", label: t("adminPanel.sidebar.bookings"), icon: "calendar_month", count: counts.bookings },
            ...
            { id: "support", label: t("adminPanel.sidebar.support"), icon: "support_agent", count: counts.support },
      ...
      >
        ...
        <span>{t("adminPanel.sidebar.logout")}</span>
      </button>
    </aside>
  );
}
