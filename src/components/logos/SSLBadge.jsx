...
const SSLBadge = () => {
  return (
    <div
      ...
      title="HTTPS transport status; encryption details depend on the current host configuration"
      role="note"
      aria-label="HTTPS transport status; this does not guarantee a specific encryption strength"
    >
      <div className="flex items-center gap-0.5">
        ...
        <span className="font-bold text-slate-800 dark:text-slate-200 text-[11px] tracking-tight ml-0.5">HTTPS</span>
      </div>
      <div className="h-3 w-px bg-slate-300 dark:bg-slate-600 mx-0.5"></div>
      <span className="text-[10px] font-bold text-slate-600 dark:text-slate-400 uppercase tracking-wider">Status</span>
    </div>
  );
};
