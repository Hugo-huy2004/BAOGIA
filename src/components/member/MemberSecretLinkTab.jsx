      ...
      {!isEditing ? (
        <div className="space-y-6">
          <div className="bg-muted/50 rounded-lg p-4 border border-border/50">
            <h4 className="text-[11px] font-black uppercase tracking-wider text-muted-foreground mb-1 flex items-center gap-1.5">
          ...
          <div className="space-y-3">
            {secretLinks.length === 0 ? (
              <div className="text-center py-10 border-2 border-dashed border-border rounded-xl">
                <span className="material-symbols-outlined text-4xl text-muted-foreground/70 mb-2">lock_open</span>
                <p className="text-xs text-zinc-500 font-medium">{t("utilities.secretLink.emptyList")}</p>
              </div>
            ) : (
              secretLinks.map((link, index) => (
                <div key={link.id} className="flex flex-col sm:flex-row sm:items-center justify-between p-4 rounded-lg border border-border bg-white dark:bg-[#1a1824] gap-4 transition-all hover:border-zinc-300 dark:hover:border-zinc-700">
                  <div className="flex items-start gap-3">
                    <div className="w-10 h-10 rounded-md bg-muted flex items-center justify-center shrink-0">
                      <span className="material-symbols-outlined text-[20px] text-muted-foreground">link</span>
                    </div>
                    <div>
                      <h4 className="text-[13px] font-black text-foreground mb-0.5 flex items-center gap-2">
                        {link.title || `Secret Link #${index + 1}`}
                        <span className="px-1.5 py-0.5 rounded text-[8px] font-black uppercase tracking-wider bg-muted text-zinc-500">
                  ...
                  <div className="flex items-center gap-2 self-end sm:self-auto">
                    <button 
                      ...
                      className="w-8 h-8 rounded-full flex items-center justify-center bg-muted hover:bg-zinc-200 dark:hover:bg-zinc-700 text-muted-foreground transition-colors"
          ...
          <button
            ...
            className={`w-full py-4 rounded-lg font-black text-[11px] uppercase tracking-widest flex items-center justify-center gap-2 transition-all ${
              secretLinks.length >= 5 
                ...
                : "bg-zinc-900 hover:bg-foreground dark:hover:bg-zinc-100 text-background shadow-md active:scale-[0.98]"
            }`}
          >
        ...
        <div className="space-y-6">
          <div className="space-y-4">
            <div className="space-y-1.5">
              ...
              <input
                ...
                className="w-full p-4 bg-muted/50 border border-border rounded-lg text-xs text-foreground focus:outline-none focus:ring-2 focus:ring-zinc-900 dark:focus:ring-white transition-all font-medium"
              />
            </div>
            <div className="space-y-1.5">
          ...
          <div className="grid grid-cols-2 gap-3">
            <button
              ...
              className="py-4 rounded-md border border-border bg-white dark:bg-[#1a1824] text-muted-foreground hover:bg-zinc-50 dark:hover:bg-zinc-900 font-black text-[11px] uppercase tracking-widest transition-colors"
            >
              {t("utilities.secretLink.btnCancel")}
            </button>
            <button
              onClick={handleSaveLink}
              className="py-4 rounded-md bg-zinc-900 hover:bg-foreground dark:hover:bg-zinc-100 text-background font-black text-[11px] uppercase tracking-widest shadow-md transition-transform active:scale-[0.98]"
            >
