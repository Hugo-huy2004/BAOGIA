...
export default function LibraryCatalog({
  ...
  return (
    <div className="space-y-4 animate-fadeIn text-left relative pb-20 select-none">
      ...
      {activeTab === "apps" && (
        <div className={`relative overflow-hidden bg-gradient-to-br ${currentSlide.gradient} border border-border/50 rounded-3xl p-5 sm:p-6 text-left shadow-sm transition-all duration-500`}>
          <div className="relative z-10 flex items-center justify-between gap-6">
            <div className="max-w-md space-y-2.5">
              <div className="inline-flex items-center gap-1.5 px-3 py-1 rounded-full text-[11px] font-medium bg-primary/15 text-primary">
                <span className="w-1.5 h-1.5 rounded-full bg-primary" />
                {currentSlide.badge}
              </div>

              <div className="space-y-1">
                <h1 className="text-lg sm:text-xl font-semibold text-foreground tracking-tight leading-snug">
                  {currentSlide.title}
                </h1>
              ...
              <div className="pt-1">
                <button
                  ...
                  className="px-5 py-2.5 bg-primary text-white font-semibold text-sm rounded-full hover:opacity-95 active:scale-95 transition-all flex items-center gap-1.5"
                >
            ...
            {/* Showcase icon (Right Side) */}
            <div
              onClick={() => handleOpenApp(currentSlide.id)}
              className="hidden sm:flex flex-col items-center justify-center w-28 h-28 rounded-2xl bg-card/60 p-3 text-center border border-border/50 cursor-pointer shrink-0 hover:scale-105 transition-transform"
            >
      ...
      <div className="relative flex items-center">
        ...
        <input
          ...
          placeholder="Tìm tiện ích, ứng dụng"
          className="w-full h-11 pl-10 pr-9 rounded-2xl border border-transparent bg-muted/60 focus:bg-background text-sm text-foreground placeholder:text-muted-foreground/60 outline-none transition-all focus:border-primary/40"
        />
        {searchQuery && (
          <button
      ...
      <div className="flex items-end justify-between px-1 pt-2">
        <div>
          <p className="text-xs font-medium text-muted-foreground uppercase tracking-wide">
            {activeTab === "installed" ? "Đã cài đặt" : "Bộ sưu tập"}
          </p>
          <h2 className="text-[26px] font-bold text-foreground tracking-tight leading-tight">
            {activeTab === "installed" ? "Ứng dụng của bạn" : "Ứng dụng nổi bật"}
          </h2>
        </div>
      ...
      {filteredApps.length === 0 ? (
        <div className="flex flex-col items-center justify-center py-12 text-center space-y-2 bg-card border border-border/60 rounded-3xl p-6">
          ...
          <div>
            <p className="text-sm font-semibold text-foreground">Không tìm thấy ứng dụng</p>
            <p className="text-sm text-muted-foreground mt-0.5">Thử từ khóa khác hoặc chuyển danh mục.</p>
          </div>
        </div>
      ) : (
          ...
          {filteredApps.map((app) => {
            ...
            return (
              ...
              >
                ...
                <div className={`w-14 h-14 rounded-[14px] shrink-0 bg-gradient-to-br ${gradient} flex items-center justify-center shadow-sm`}>
                ...
                <div className="shrink-0 flex flex-col items-center gap-1">
                  ...
                  {app.badge && (
                    <span className="text-[9px] font-medium text-muted-foreground/70 uppercase tracking-wide">{app.badge}</span>
                  )}
                </div>
              </div>
            );
        ...
        <div className="max-w-md mx-auto flex items-center justify-around">
          <button
            ...
            className={`flex flex-col items-center gap-0.5 px-4 py-1 rounded-xl transition-all ${
              activeTab === "apps" ? "text-primary font-semibold" : "text-muted-foreground hover:text-foreground font-medium"
            }`}
          >
            <span className="material-symbols-outlined text-xl" style={{ fontVariationSettings: activeTab === "apps" ? "'FILL' 1" : "" }}>apps</span>
            <span className="text-[11px]">Ứng dụng</span>
          </button>

          <button
            onClick={() => setActiveTab("installed")}
            className={`flex flex-col items-center gap-0.5 px-4 py-1 rounded-xl transition-all ${
              activeTab === "installed" ? "text-primary font-semibold" : "text-muted-foreground hover:text-foreground font-medium"
            }`}
          >
            ...
            <span className="text-[11px]">Đã cài ({installedApps.length})</span>
          </button>
        </div>
      </div>
      ...
      {pendingInstallApp && (
        <div className="fixed inset-0 bg-black/40 backdrop-blur-md flex items-center justify-center z-[600] p-4 text-center animate-fadeIn">
          <div className="bg-card border border-border/60 rounded-3xl p-6 max-w-sm w-full space-y-4 shadow-2xl animate-slideUp text-left">
            <div className={`w-14 h-14 rounded-2xl bg-gradient-to-br ${gradients[pendingInstallApp.tint] || gradients.indigo} flex items-center justify-center text-white mx-auto shadow-sm`}>
            ...
            <div className="text-center space-y-1">
              ...
              <p className="text-sm text-muted-foreground leading-relaxed">
                Cả hai lựa chọn đều cài ứng dụng như nhau, chỉ khác việc có icon riêng ở màn hình chính hay không.
              </p>
            </div>
      ...
      {selectedApp && (
        <div className="fixed inset-0 bg-black/40 backdrop-blur-md flex items-end sm:items-center justify-center z-[500] p-0 sm:p-4 transition-opacity">
          ...
          <div className="relative w-full sm:max-w-lg bg-card border-t sm:border border-border/60 rounded-t-3xl sm:rounded-3xl p-6 shadow-2xl z-[510] max-h-[90vh] overflow-y-auto animate-slideUp text-left space-y-6">
            ...
            <div className="space-y-2">
              <h4 className="text-sm font-medium text-muted-foreground">Mô tả ứng dụng</h4>
            ...
            <div className="grid grid-cols-3 gap-2 py-2 border-y border-border/50 text-center">
              ...
              <div className="p-2 rounded-xl bg-muted/40">
                <span className="text-xs text-muted-foreground block">Độ tuổi</span>
                <span className="text-sm font-semibold text-foreground">4+</span>
              </div>
              <div className="p-2 rounded-xl bg-muted/40">
                <span className="text-xs text-muted-foreground block">Khả dụng</span>
            ...
            <div className="space-y-2 pt-2">
              ...
              >
                ...
                <span>Tạo shortcut màn hình chính</span>
              </button>
            </div>
          </div>
        </div>
