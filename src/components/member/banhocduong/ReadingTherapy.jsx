...
export default function ReadingTherapy({ onBack, onCompleteActivity, showToast, bio }) {
  ...
  return (
    <div className="max-w-xl mx-auto space-y-5 animate-scaleUp bg-gradient-to-br from-background via-muted to-primary/20 text-foreground p-6 rounded-3xl border border-border shadow-2xl relative overflow-hidden">
      ...
      {/* Header */}
      <div className="relative z-10 flex items-center justify-between border-b pb-2.5 border-border/60">
        <button type="button" onClick={() => { stopNarration(); onBack(); }} className="text-muted-foreground text-[10px] font-black uppercase tracking-wider hover:text-foreground transition-colors">
          Quay lại thẻ
        </button>
        <span className="text-[10px] font-black uppercase text-primary tracking-wider">Trị liệu Đọc & Nghe</span>
      </div>
          ...
          <div className="space-y-4 bg-white/5 border border-white/10 p-4 rounded-3xl">
            {therapyType === "story" ? (
              <div className="space-y-3">
                <label className="text-[9.5px] font-black uppercase tracking-wider text-muted-foreground">Cậu cảm thấy thế nào hôm nay?</label>
                <div className="grid grid-cols-3 gap-2">
                  {MOODS.map(m => (
                    ...
                    >
                      {React.createElement(m.icon, { className: "w-3.5 h-3.5 text-primary shrink-0" })}
                      <span>{m.label}</span>
                    </button>
                  ))}
                </div>
                ...
                <div className="space-y-1.5 pt-2">
                  <label className="text-[9.5px] font-black uppercase tracking-wider text-muted-foreground">Kể thêm cho AI nghe điều đang xảy ra (tùy chọn)</label>
                  <textarea
                    placeholder="Ví dụ: Tớ vừa trải qua một kỳ thi căng thẳng, tớ có chút mệt mỏi với các mối quan hệ..."
                    value={customContext}
                    onChange={e => setCustomContext(e.target.value)}
                    className="w-full h-24 p-3 border border-white/10 bg-white/5 text-xs rounded-xl outline-none focus:ring-2 ring-primary/50 transition-all font-medium text-white placeholder-muted-foreground/60"
                  />
                </div>
              </div>
            ) : (
              <div className="space-y-3">
                <div className="space-y-1.5">
                  <label className="text-[9.5px] font-black uppercase tracking-wider text-muted-foreground">Ghi lại giấc mơ chi tiết của cậu</label>
                  <textarea
                    placeholder="Ví dụ: Tớ mơ thấy mình đang bay giữa những đám mây ngũ sắc nhưng rồi đột ngột rơi xuống mặt nước yên tĩnh..."
                    value={customContext}
                    onChange={e => setCustomContext(e.target.value)}
                    className="w-full h-36 p-3 border border-white/10 bg-white/5 text-xs rounded-xl outline-none focus:ring-2 ring-primary/50 transition-all font-medium text-white placeholder-muted-foreground/60"
                  />
                </div>
              </div>
            )}
            ...
            <button
              ...
              className="w-full py-3 bg-primary hover:bg-primary/90 text-white text-[10px] font-black uppercase tracking-wider rounded-xl transition-all shadow-md active:scale-95 flex items-center justify-center gap-2 border border-primary/30"
            >
      ...
      {step === "loading" && (
        <div className="py-20 text-center space-y-4 relative z-10">
          <div className="relative w-16 h-16 mx-auto">
            ...
            <BookOpen className="w-6 h-6 text-primary absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 animate-pulse" />
          </div>
          <div>
            <p className="text-[11px] font-black text-foreground">Hugo Studio AI đang sáng tác...</p>
            <p className="text-[9.5px] text-muted-foreground mt-1">Quá trình này có thể mất từ 10 - 20 giây để trau chuốt câu từ.</p>
          </div>
        </div>
      )}
      ...
      {step === "reading" && aiStory && (
        <div className="space-y-5 text-left relative z-10">
          ...
          <div className="bg-white/5 border border-white/10 rounded-3xl p-6 shadow-lg relative min-h-[400px]">
            ...
            <div className="flex justify-between items-start border-b border-white/10 pb-3 mb-4">
              <div>
                ...
                <h2 className="text-[16px] font-black text-foreground font-serif leading-tight mt-1">{aiStory.title}</h2>
              </div>
              <span className="text-[9px] font-mono font-black text-muted-foreground flex items-center gap-1 shrink-0">
            ...
            {/* Paragraph view */}
            <div className="space-y-4 font-serif text-[12px] text-muted-foreground leading-relaxed font-medium">
              {paragraphs.map((para, idx) => (
                <p
                  ...
                  className={`p-2 rounded-xl transition-all cursor-pointer ${
                    currentParaIdx === idx
                      ? "bg-primary/10 text-white border-l-4 border-primary pl-3 font-semibold shadow-sm"
                      : "hover:bg-white/5"
                  }`}
                >
                  {para}
          ...
          <div className="grid grid-cols-1 md:grid-cols-12 gap-4">
            ...
            <div className="md:col-span-6 bg-white/5 border border-white/10 p-4 rounded-3xl flex flex-col justify-between gap-3 shadow-sm">
              <div className="flex items-center justify-between">
                <span className="text-[9px] font-black uppercase tracking-wider text-muted-foreground">Giọng đọc kể chuyện AI</span>
                {isPlayingNarration && <span className="w-1.5 h-1.5 rounded-full bg-primary animate-ping" />}
              </div>

              <div className="flex items-center gap-3">
                <button
                  onClick={handleToggleNarration}
                  className="w-10 h-10 rounded-full bg-primary hover:bg-primary/90 text-white flex items-center justify-center shadow-md active:scale-95 transition-all animate-none"
                >
                  {isPlayingNarration ? <Pause className="w-4 h-4" /> : <Play className="w-4 h-4 pl-0.5" />}
                </button>
                <button
                  onClick={stopNarration}
                  className="w-10 h-10 rounded-full border border-white/10 text-muted-foreground bg-white/5 hover:bg-white/10 flex items-center justify-center active:scale-95 transition-all"
                >
                ...
                <div className="flex-1 min-w-0">
                  ...
                  <p className="text-[8px] text-muted-foreground">Nhấp chọn đoạn văn bất kỳ để đọc từ đó</p>
                </div>
              </div>

              {/* Speed slider */}
              <div className="flex items-center gap-2 pt-1 border-t border-white/5">
                <span className="text-[8.5px] font-black text-muted-foreground uppercase w-14 shrink-0">Tốc độ:</span>
                <input
                  type="range"
                  min="0.6"
                  max="1.2"
                ...
                />
                <span className="text-[9px] font-mono font-bold text-muted-foreground">{narrationSpeed}x</span>
              </div>
            </div>
            ...
            <div className="md:col-span-6 bg-white/5 border border-white/10 p-4 rounded-3xl space-y-3 shadow-sm">
              <span className="text-[9px] font-black uppercase tracking-wider text-muted-foreground block">Âm thanh thiên nhiên nền</span>
              ...
              <div className="grid grid-cols-2 gap-2">
                {[
                  ...
                  <div key={sound.key} className="flex flex-col gap-1 p-2 rounded-2xl bg-white/5 border border-white/10">
                    <div className="flex items-center justify-between">
                      <span className="text-[9px] font-bold text-muted-foreground flex items-center gap-1.5">
          ...
          <button
            ...
            className="w-full py-3 bg-white text-foreground hover:bg-muted text-[10px] font-black uppercase tracking-wider rounded-xl transition-all shadow-md active:scale-95 text-center block border border-white/20"
          >
