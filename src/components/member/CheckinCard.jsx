...
export default function CheckinCard({ email, showToast }) {
  ...
  return (
    <div className="bg-gradient-to-br from-rose-50 to-amber-50 dark:from-rose-500/10 dark:to-amber-500/5 rounded-3xl border border-rose-200/60 dark:border-rose-500/20 p-5 space-y-4">
      <div className="flex items-center justify-between">
        <div>
          <h3 className="text-sm font-black text-zinc-900 dark:text-white flex items-center gap-1.5">
            <span className="material-symbols-outlined text-rose-500 text-base">event_available</span>
            {t("memberPortal.checkin.title")}
          </h3>
          <p className="text-[10px] text-zinc-500 dark:text-zinc-400">{t("memberPortal.checkin.subtitle")}</p>
        </div>
        {status.weekLocked && (
          <span className="text-[9px] font-black uppercase px-2 py-1 rounded-full bg-red-100 text-red-600 dark:bg-red-950/30 dark:text-red-400">
          ...
          return (
            <div
              ...
              className={`flex flex-col items-center gap-1 py-2.5 rounded-xl border text-center transition-all ${
                claimed
                  ...
                  : isToday
                    ? "bg-white dark:bg-zinc-800 border-rose-400 ring-2 ring-rose-300/50"
            ...
            >
              ...
              {claimed ? (
                ...
                <span className="material-symbols-outlined text-amber-500 text-base">paid</span>
              )}
      ...
      <div className="flex items-center gap-2">
        {[
          ...
          <div key={m.threshold} className={`flex-1 flex items-center justify-between gap-1 px-3 py-2 rounded-xl border text-[10px] ${
            m.awarded ? "bg-emerald-500/10 border-emerald-400/40" : "bg-white/60 dark:bg-zinc-900/30 border-zinc-200/60 dark:border-zinc-800/60"
          }`}>
            <span className="font-bold text-zinc-600 dark:text-zinc-300">{t("memberPortal.checkin.milestone", { days: m.threshold })}</span>
            <span className="font-mono font-bold text-amber-600 dark:text-amber-400">+{m.bonus}</span>
          </div>
        ))}
      </div>
      ...
      <div className="flex items-center justify-between gap-3">
        ...
        <button
          ...
          className="px-5 py-2.5 rounded-xl bg-rose-500 hover:bg-rose-600 text-white text-xs font-bold uppercase tracking-wider transition-all active:scale-95 disabled:opacity-40 disabled:cursor-not-allowed shrink-0"
        >
          {claiming
            ? "..."
