      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6 items-start">
        ...
        <div className="bg-white dark:bg-[#12111a] rounded-[2rem] border border-zinc-200/50 dark:border-zinc-800/60 p-6 shadow-sm space-y-6">
              ...
              {QR_TYPES.map(tp => {
                ...
                return (
                  <button key={tp.id} onClick={() => setQrType(tp.id)}
                    className={`flex flex-col items-center gap-2 p-3.5 rounded-2xl border transition-all active:scale-95 text-center ${
          ...
          <div>
            ...
            <div className="bg-zinc-50 dark:bg-zinc-900/40 p-5 rounded-2xl border border-zinc-100 dark:border-zinc-800/50">
              {qrType === "url" && (
                  <div className="space-y-2">
                      ...
                      <div className="px-4 py-3 bg-white dark:bg-[#12111a] border border-zinc-200 dark:border-zinc-800 rounded-xl text-xs font-bold text-zinc-700 dark:text-zinc-300 font-mono break-all select-all shadow-inner">{publicLink}</div>
              ...
              {qrType === "vcard" && (
                  <div className="space-y-3">
                      ...
                      <div className="grid grid-cols-2 gap-3 px-4 py-3 bg-white dark:bg-[#12111a] border border-zinc-200 dark:border-zinc-800 rounded-xl shadow-inner text-[11px]">
                  ...
                  <div className="space-y-4">
                      ...
                      <div className="grid grid-cols-1 gap-4">
                          <div className="space-y-1.5">
                              ...
                              <input value={wifiSsid} onChange={e => setWifiSsid(e.target.value)} placeholder="VD: ThuVien_Truong..." className="w-full px-4 py-2.5 rounded-xl border border-zinc-200 dark:border-zinc-800 bg-white dark:bg-[#12111a] text-xs focus:outline-none focus:ring-2 focus:ring-emerald-500/50 transition-shadow font-bold" />
                          </div>
                          <div className="space-y-1.5">
                              <label className="text-[9.5px] font-bold uppercase tracking-wider text-zinc-500">Mật Khẩu</label>
                              <input value={wifiPass} onChange={e => setWifiPass(e.target.value)} placeholder="••••••••" type="password" className="w-full px-4 py-2.5 rounded-xl border border-zinc-200 dark:border-zinc-800 bg-white dark:bg-[#12111a] text-xs focus:outline-none focus:ring-2 focus:ring-emerald-500/50 transition-shadow font-bold" />
                          </div>
                      </div>
              ...
              {qrType === "text" && (
                  <div className="space-y-2">
                      ...
                      <textarea value={customText} onChange={e => setCustomText(e.target.value)} placeholder="Dán link Google Drive, Docs hoặc văn bản ghi chú vào đây..." rows={3} className="w-full px-4 py-3 rounded-xl border border-zinc-200 dark:border-zinc-800 bg-white dark:bg-[#12111a] text-xs focus:outline-none focus:ring-2 focus:ring-amber-500/50 transition-shadow font-mono resize-none" />
        ...
        <div className="flex flex-col items-center justify-center p-8 bg-zinc-50 dark:bg-zinc-900/20 rounded-[2rem] border border-dashed border-zinc-300 dark:border-zinc-800/80 min-h-[400px]">
            <AnimatePresence mode="wait">
              <motion.div 
                ...
                className="relative"
              >
                <div ref={qrRef} className="bg-white p-6 rounded-3xl shadow-xl border border-zinc-200">
                  <QRCodeSVG 
                      value={getQRData()} 
                      size={220} 
                      level="H" 
                ...
                {/* Floating icon */}
                <div className={`absolute -bottom-4 -right-4 w-12 h-12 rounded-2xl ${activeColor.bg} ${activeColor.text} ${activeColor.border} border shadow-lg flex items-center justify-center backdrop-blur-md`}>
            ...
            <button 
              ...
              className={`mt-10 px-8 py-3.5 rounded-2xl text-white font-black text-xs uppercase tracking-widest shadow-lg transition-all active:scale-[0.98] flex items-center gap-2 bg-gradient-to-r hover:brightness-110
