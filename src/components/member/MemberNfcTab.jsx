...
const COLOR_MAP = {
  indigo:  { bg: "bg-primary/10", border: "border-primary/30", text: "text-primary" },
  rose:    { bg: "bg-rose-500/10",   border: "border-rose-400/30",   text: "text-rose-500" },
  emerald: { bg: "bg-success/10",border: "border-success/30",text: "text-success" },
  ...
  return (
    <div className="space-y-6">
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6 items-start">
        ...
        {/* LEFT: Generator Form */}
        <div className="bg-white dark:bg-background rounded-[2rem] border border-border/50 p-6 shadow-sm space-y-6">
            ...
            <div className="bg-muted/50 p-5 rounded-lg border border-border/60">
              {qrType === "url" && (
                  <div className="space-y-2">
                      <div className="text-[10px] font-black uppercase tracking-widest text-primary flex items-center gap-1.5"><span className="material-symbols-outlined text-xs">public</span> {t("utilities.qrCode.profileUrlLabel", "URL HỒ SƠ CHÍNH")}</div>
                      <div className="px-4 py-3 bg-white dark:bg-background border border-border rounded-md text-xs font-bold text-foreground/80 font-mono break-all select-all shadow-inner">{publicLink}</div>
              ...
              {qrType === "vcard" && (
                  <div className="space-y-3">
                      ...
                      <div className="grid grid-cols-2 gap-3 px-4 py-3 bg-white dark:bg-background border border-border rounded-md shadow-inner text-[11px]">
                          <div><span className="text-zinc-400 font-bold">{t("utilities.vcard.fields.firstName", "Họ tên")}:</span> <span className="font-black text-foreground">{bio?.displayName || "—"}</span></div>
                          <div><span className="text-zinc-400 font-bold">{t("utilities.vcard.fields.phone", "SĐT")}:</span> <span className="font-black text-foreground">{bio?.phone || "—"}</span></div>
                          <div className="col-span-2 truncate"><span className="text-zinc-400 font-bold">{t("utilities.vcard.fields.email", "Email")}:</span> <span className="font-black text-foreground">{bio?.contactEmail || bio?.email || "—"}</span></div>
                      </div>
              ...
              {qrType === "wifi" && (
                  <div className="space-y-4">
                      <div className="text-[10px] font-black uppercase tracking-widest text-success flex items-center gap-1.5"><span className="material-symbols-outlined text-xs">router</span> {t("utilities.qrCode.wifiLabel", "CHIA SẺ WIFI HỌC NHÓM")}</div>
                      <div className="grid grid-cols-1 gap-4">
                          <div className="space-y-1.5">
                              ...
                              <input value={wifiSsid} onChange={e => setWifiSsid(e.target.value)} placeholder="SSID..." className="w-full px-4 py-2.5 rounded-md border border-border bg-white dark:bg-background text-xs focus:outline-none focus:ring-2 focus:ring-success/50 transition-shadow font-bold" />
                          </div>
                          <div className="space-y-1.5">
                              ...
                              <input value={wifiPass} onChange={e => setWifiPass(e.target.value)} placeholder="••••••••" type="password" className="w-full px-4 py-2.5 rounded-md border border-border bg-white dark:bg-background text-xs focus:outline-none focus:ring-2 focus:ring-success/50 transition-shadow font-bold" />
                          </div>
                      </div>
              ...
              {qrType === "text" && (
                  <div className="space-y-2">
                      <div className="text-[10px] font-black uppercase tracking-widest text-warning flex items-center gap-1.5"><span className="material-symbols-outlined text-xs">edit_document</span> {t("utilities.qrCode.customTextLabel", "LINK NHÓM / TÀI LIỆU")}</div>
                      <textarea value={customText} onChange={e => setCustomText(e.target.value)} placeholder="..." rows={3} className="w-full px-4 py-3 rounded-md border border-border bg-white dark:bg-background text-xs focus:outline-none focus:ring-2 focus:ring-warning/50 transition-shadow font-mono resize-none" />
        ...
        <div className="flex flex-col items-center justify-center p-8 bg-muted/50 rounded-[2rem] border border-dashed border-border min-h-[400px]">
            ...
            <button 
              ...
              className={`mt-10 px-8 py-3.5 rounded-lg text-white font-black text-xs uppercase tracking-widest shadow-lg transition-all active:scale-[0.98] flex items-center gap-2 bg-gradient-to-r hover:brightness-110
                ${qrType === 'url' ? 'from-primary to-purple-600 shadow-primary/25' : ''}
                ${qrType === 'vcard' ? 'from-rose-500 to-pink-600 shadow-rose-500/25' : ''}
                ${qrType === 'wifi' ? 'from-success to-teal-500 shadow-success/25' : ''}
                ${qrType === 'text' ? 'from-warning to-orange-500 shadow-warning/25' : ''}
              `}
            >
