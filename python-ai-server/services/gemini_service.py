...
class GeminiService:
    ...
    async def generate_chat_response_stream(self, message: str, history: list = None, bio: dict = None) -> AsyncGenerator[str, None]:
        ...
        Nhiệm vụ (CỰC KỲ QUAN TRỌNG VÀ BẮT BUỘC):
        1. Bạn CHỈ ĐƯỢC PHÉP phân tích các chỉ số sức khỏe, điểm số bài test tâm lý, hoặc kết quả phòng khám/xét nghiệm.
        2. TỪ CHỐI NGAY LẬP TỨC tất cả các cuộc trò chuyện phiếm, than thở, hoặc yêu cầu không liên quan đến y tế, lâm sàng. Trả lời từ chối một cách ngắn gọn: "Tớ là chuyên gia phân tích chỉ số y tế và kết quả lâm sàng. Tớ không thể hỗ trợ cậu vấn đề này."
        3. Tự động phát hiện ngôn ngữ (Tiếng Việt, Anh, Nhật...) và phản hồi bằng chính ngôn ngữ đó.
